...
def main():
    ...
    test_fits_normal = "/home/esker7293/Cluster_formation/Hotdisk/HiPPOS/HiPPOS.G010.3009-00.1477.Band6.TM1+TM2.contin.combselfcal.image.tt0.fits" # 请修改为真实存在的文件
    test_fits_rbm05 = "/home/esker7293/Cluster_formation/Hotdisk/HiPPOS/HiPPOS.G010.3009-00.1477.Band6.TM1+TM2.contin.combselfcal.robust-0.5.image.tt0.fits" # 请修改为真实存在的文件
    test_csv_path = os.path.join(os.path.dirname(__file__), 'G010_Sources_Band6_cycle11_venn_sources.csv') # "g339_b6_c7_venn_sources_v2_less.csv"
    output_directory = os.path.join(os.path.dirname(__file__), "test_output_G010") 
    ...
    fitter = BatchImfitter(
        ...
        distance_pc=2400.0, # 2170.0,
        cluster_name="G010" #"G339_cycle7_test"
    )
    ...
    std_normal = 1.69e-5 # 加快测试速度，直接用Carta测量的结果，实际使用时建议用 sigma_clipped_stats 重新测量
    std_rbm05 = 2.75e-5 # 加快测试速度，直接用Carta测量的结果，实际使用时建议用 sigma_clipped_stats 重新测量
    
    # 假设你只测试 normal 图，不传入 rbm05 
    final_results = fitter.run_pipeline(
        clustername="G010", #"G339_cycle7_test",
